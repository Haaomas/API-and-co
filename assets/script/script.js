...
fetch(endpoint_the_office)
  ...
  .then((cleanData) => {
    ...
    function searchQuotes(e) {
      ...
      console.log(
        "🤖 ~ file: script.js ~ line 41 ~ searchQuotes ~ searchInput",
        searchInput
      );

      //Condition of the research (in the firstname or in the lastname)
      const searchArray = cleanData.filter(
        (pseudo) =>
          pseudo.character.firstname.toLowerCase().includes(searchInput) ||
          pseudo.character.lastname.toLowerCase().includes(searchInput)
      );
